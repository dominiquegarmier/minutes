...
def map_continuous_to_device_id(continuous_id: int) -> int:
    ...
    )  # Fallback to same ID if not found
...
class AudioRecorder:
                ...
                with self._device_locks[device_id]:
                    if self._device_bufs[device_id]:
                        chunk = np.concatenate(
                            list(self._device_bufs[device_id]), axis=0
                        )
        ...
        def run() -> None:
            ...
            try:
                with sd.InputStream(**stream_kwargs):
                    logging.info(
                        "Audio stream started for device %d: %s",
                        device_id,
                        dev_info["name"],
                    )
                    while not self._stop.is_set():
                ...
                logging.error(
                    "Failed to start audio stream for device %d: %s", device_id, e
                )

        thread = threading.Thread(
            target=run, name=f"AudioRecorder-{device_id}", daemon=True
        )
        thread.start()
        ...
        for device_id in device_list:
            ...
            if ffmpeg_device is not None:
                cmd = [
                    ...
                    "avfoundation",
                    "-i",
                    f":{ffmpeg_device}",  # Use mapped ffmpeg device ID
                    "-ac",
                    "2",
                    "-ar",
                ...
                try:
                    ...
                    logging.info(
                        "Started ffmpeg recording for device %d (%s) -> ffmpeg device %d",
                        device_id,
                        device_name,
                        ffmpeg_device,
                    )
                    ...
                    logging.error(
                        "Failed to start ffmpeg for device %d: %s", device_id, e
                    )
            else:
                logging.warning(
                    "Device %d (%s) not found in ffmpeg devices", device_id, device_name
                )
    ...
    def _segment_generator_single(self) -> Iterable[Path]:
        ...
        hop_samples = int(
            max(1, (self.segment_secs - self.overlap_secs) * self.cfg.sample_rate)
        )
        ...
        while not self._stop.is_set():
            ...
            if (
                accum.shape[0] - last_emit >= hop_samples
                and accum.shape[0] >= segment_samples
            ):
    ...
    def _segment_generator_multidevice(self) -> Iterable[Path]:
        ...
        while not self._stop.is_set():
            time.sleep(
                self.segment_secs - self.overlap_secs
            )  # Wait for segment duration

            # Combine all device audio files into one segment
            combined_file = (
                Path(tempfile.gettempdir()) / f"combined_{uuid.uuid4().hex}.wav"
            )
            ...
            if input_args:
                cmd = (
                    ...
                    + [
                        "-filter_complex",
                        f"amix=inputs={len(self._temp_files)}:duration=shortest",
                        "-ac",
                        "1",
                        "-ar",
                        "16000",  # Mono, 16kHz for whisper
                        str(combined_file),
                    ]
                )
