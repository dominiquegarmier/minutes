...
class TestOllamaConfig:
    ...
    def test_custom_config(self) -> None:
        ...
        config = OllamaConfig(
            model="llama2:7b", host="http://localhost:8080", timeout=120.0
        )
...
class TestSummarizer:
    ...
    def test_successful_summarization(self, mock_post: MagicMock) -> None:
        ...
        assert (
            result == "- Discussed project timeline\n- Assigned tasks to team members"
        )
    ...
    def test_custom_model_and_host(self, mock_post: MagicMock) -> None:
        ...
        config = OllamaConfig(
            model="custom-model:latest", host="http://custom-host:9999"
        )
        summarizer = Summarizer(config)

        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {"message": {"content": "Summary content"}}
        mock_post.return_value = mock_response
